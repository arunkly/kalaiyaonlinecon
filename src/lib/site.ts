...
export function normalizeWebsite(raw: unknown, fallback = DEFAULT_SITE.website) {
...
function parseChrome(raw: unknown): { bottomBar: ChromeKey[]; footerMenu: ChromeKey[]; website: string } {
...
function normalize(
  row: Partial<Omit<SiteIdentity, "bottomBar" | "footerMenu">> & {
    chrome?: unknown;
    bottomBar?: string[];
    footerMenu?: string[];
  } | null | undefined,
): SiteIdentity {
  const chrome = parseChrome(
    row && "chrome" in (row as object)
      ? (row as { chrome?: unknown }).chrome
      : { bottomBar: row?.bottomBar, footerMenu: row?.footerMenu, website: row?.website },
  );
  return {
    ...
    website: normalizeWebsite(row?.website || chrome.website, DEFAULT_SITE.website),
...
export const saveSiteIdentity = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const chrome = JSON.stringify({
      bottomBar: next.bottomBar,
      footerMenu: next.footerMenu,
    ...
    try {
      await sql`
        update seo_settings set site_name = ${next.name}, canonical_url = ${next.website} where id = 1
      `;
    } catch {
    ...
    try {
      await sql`
        update about_page set org_name = ${next.name}, website = ${next.website} where id = 1
      `;
    } catch {
